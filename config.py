    ...
    def on_train_epoch_start(self, trainer, pl_module):
        for idx, optimizer in enumerate(trainer.optimizers):
            for param_group in optimizer.param_groups:
                lr = param_group["lr"]
                print(
                    f"Epoch {trainer.current_epoch}: Optimizer {idx} Learning Rate = {lr}"
                )
...
def get_config():
    return {
        ...
        "min_pred_idx": 1941,
        "batch_size": 4,  # can change to accommodate for larger models
        "val_batch_size": 64,
        # tft params
        "tft_params": {
            "hidden_size": 512,  # can change
            "lstm_layers": 8,  # can change
            "dropout": 0.4,  # can change
            "output_size": 7,  # number of quantiles to map to
            "loss": QuantileLoss(quantiles=[0.02, 0.1, 0.25, 0.5, 0.75, 0.9, 0.98]),
            "attention_head_size": 32,  # can change
            "learning_rate": 0.1,  # can change
            "hidden_continuous_size": 128,  # can change
            "hidden_continuous_sizes": {},
            "log_interval": 10,
            "log_val_interval": 10,
            "log_gradient_flow": False,
            "optimizer": "Ranger",
            "reduce_on_plateau_patience": 2,  # if performance doesn't increase for 2 epochs, reduce learning rate
            # "reduce_on_plateau_reduction":0.1,  # by this factor
